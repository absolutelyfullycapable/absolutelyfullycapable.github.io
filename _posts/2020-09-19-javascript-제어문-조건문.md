...
ex 3) 특정 년도가 윤년인지 아닌지 판정하는 함수 (윤년은 4호 나누어떨어지고, 100으로는 나누어떨어지지 않지만 400으로는 나누어떨어지는 해)
...
ex 4) 로그인 (아이디: admin, 패스워드: 123456)
...
- **switch(상수값(표현식))**
	...
	- C, Java에서는 case 뒤 반드시 상수값을 써야 하지만 자바스크립트에서는 표현식을 그대로 작성 가능함
		- 하지만 표현식을 쓰면 알고리즘 파악이 어려워 **숫자 리터럴** 또는 **문자열 리터럴**을 사용하는 게 좋음
...
- **break**
	- 해당 case문의 문장을 처리하고 블록을 빠져나오도록 하고 다음 작업을 시작함
