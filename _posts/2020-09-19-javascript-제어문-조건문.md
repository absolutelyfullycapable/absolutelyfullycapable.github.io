...
3) 특정 년도가 윤년인지 아닌지 판정하는 함수 (윤년은 4호 나누어떨어지고, 100으로는 나누어떨어지지 않지만 400으로는 나누어떨어지는 해)
...
- **case 표현식(case 라벨의 값) ~ default**
	...
	- switch 불록 어느 위치에 있어도 문법 오류가 발생하지 않지만, 블록 중간에 있으면 프로그램을 이해하기 어려워 되도록이면 마지막에 작성하도록 함
